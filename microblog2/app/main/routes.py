...
from app import db
from app.main.forms import EditProfileForm, EmptyForm, PostForm, CloseAccountForm, SearchForm
...
def search():
    ...
    prev_url = url_for('main.search', q=g.search_form.q.data, page=page - 1) \
        if page > 1 else None
    return render_template('search.html', title =_('Search'), search = g.search_form.q.data, total=total, posts=posts, next_url=next_url, prev_url=prev_url)
...
def index():  # view function
    ...
    return render_template("index.html", title=_("Home"), form=form, posts=posts.items, next_url=next_url, prev_url=prev_url) #render_template renders code in html. #colored 'posts' contains all the user's posts, 'posts' can be accessed by 'index.html' to print data to the page usually using the "." operator
...
def user(username):
    ...
    prev_url = url_for('main.user', username=user.username, page=posts.prev_num) \
        if posts.has_prev else None #prev_url is assigned the link to the prev page, if no previous page available, link is not shown
    form = EmptyForm()
    return render_template('user.html', title=_("%(username)s's Profile", username=user.username.title() ), user=user, posts=posts.items, next_url=next_url, prev_url=prev_url, form=form) #colored 'user' contains all the user's data and colored 'posts' contains all the user's posts. Both 'user' and 'posts' can be accessed by 'user.html' to print data to the page usually using the "." operator
